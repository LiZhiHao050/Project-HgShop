...
<div class="container-fluid">
    <div class="row">
        <form action="category/categoryList" method="post">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button type="button" class="btn btn-primary btn-md" data-toggle="modal" data-target="#myModal2"
                                onclick="getCategoryById(${category.id},1);">修改
                        </button>
                        <button type="button" class="btn btn-info btn-md" data-toggle="modal" data-target="#myModal3"
                                onclick="getCategoryById(${category.id},2);">查看
                        </button>
                    </td>
                </tr>
            </c:forEach>
        ...
        <nav>
            <ul class="pagination">
                <c:if test="${pageInfo.hasPreviousPage}">
                    <li>
                        <a href="category/categoryList?pageNum=${pageInfo.prePage}&name=${category.name}">
                ...
                <c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
                    <c:if test="${pageInfo.pageNum==pageNum}">
                        <li class="active"><a
                                href="category/categoryList?pageNum=${pageNum}&name=${category.name}">${pageNum}</a></li>
                    </c:if>
                    <c:if test="${pageInfo.pageNum!=pageNum}">
                        <li><a href="category/categoryList?pageNum=${pageNum}&name=${category.name}">${pageNum}</a></li>
                    </c:if>
                </c:forEach>
                <c:if test="${pageInfo.hasNextPage}">
                    <li>
                        <a href="category/categoryList?pageNum=${pageInfo.nextPage}&name=${category.name}" aria-label="Next">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close" onclick="$('#tree').hide()"><span
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal" onclick="$('#tree').hide()">关闭
                </button>
...
<div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
                ...
                <form class="form-horizontal" id="modalForm2" action="javascript:void(0);">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            ...
                            <input type="text" class="form-control" id="parentNameparentName2" placeholder="选择父分类">
                            <div id="tree2"
                                 style="display: none; position:absolute; z-index:1010; background-color:white;"></div>
                        </div>
                    </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal" onclick="$('#tree2').hide()">关闭
                </button>
...
<script>
    function addCategory(){
        ...
        $("#tree").hide();//选中树节点后隐藏树
        $.post('category/addCategory',$('#modalForm1').serialize(),function(data){
            if(data){
                $("#myModal").modal('hide');
    ...
    function editCategory(){
        ...
        $.post('category/addCategory',$('#modalForm2').serialize(),function(data){
            if(data){
                $("#myModal2").modal('hide');
                window.location.reload();
            }else{
    ...
    function getCategoryById(id,flag){
        $.post('category/getCategoryById',{id:id},function(data){
            if(flag==1){
                //修改模态框的数据回显
                $('#id2').val(data.id);
                $('#name2').val(data.text);
    ...
    $(function(){
        $("#parentName").click(function() {
            $.post('category/getAllCategories', {}, function(data) {
                var options = {
                    levels : 2,
                    data : data,
